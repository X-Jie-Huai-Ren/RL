...
class DQNAgent:
    def __init__(self, q_func, replay_buffer, batch_size, replay_start_size, optimizer, n_acts, update_target_steps, explorer, eps=0.1, gamma=0.9) -> None:
    ...
    def predict(self, observation):
        ...
        Q_lst = self.pred_func(observation)
    ...
    def learn_batch(self, obs_batch, action_batch, reward_batch, next_obs_batch, done_batch):
        ...
        pred_Vs = self.pred_func(obs_batch)   # batch_size是32,且在"CartPole-v0"中由2种action 则pred_Vs是32x2的矩阵
        ...
        target_Q = reward_batch + (1-done_batch) * self.gamma * self.pred_func(next_obs_batch).max(1)[0]
    ...
    def sync_target(self):
        for target_params, params in zip(self.target_func.parameters(), self.pred_func.parameters()):

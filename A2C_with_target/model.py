...
# 价值网络
# 对于A2C算法，对actor网络(策略网络)的训练还是基于最大action-value的，也就是传统的DQN训练模式
# 而对于critic网络(价值网络)的训练不是基于传统AC算法的action-vlaue(所以输出维度是action_dim)，而是state-value(输出维度为1)
class ValueNet(nn.Module):

    def __init__(self, in_channels, channels_lst, output_dim=1) -> None:
